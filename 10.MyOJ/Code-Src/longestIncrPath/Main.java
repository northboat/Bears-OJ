...
class Main{
	...
	public static void main(String[] args){
		...
		for(i = 0; i < data.length; i++){
			...
			int[][] realData = new int[srcData.length][srcData[0].trim().split(" ").length];
			for(int j = 0; j < srcData.length; j++){
				String[] row = srcData[j].trim().split(" ");
				for(int k = 0; k < row.length; k++){
					realData[j][k] = Integer.parseInt(row[k]);
				}
			}
			int realAns = Integer.parseInt(a.trim());
			
			long startTime = System.currentTimeMillis();
			Solution solution = new Solution();
			
			//the statement should be writen own
			int curAns = solution.longestIncrPath(realData);
