...
//枚举左值类型:错误，数值，开始符，函数，S-Expression，Q-Expression 
enum{ LVAL_ERR, LVAL_NUM, LVAL_SYM, LVAL_FUN, LVAL_SEXPR, LVAL_QEXPR, LVAL_CLS };
...
void lval_print(lval* v){
	switch(v->type){
		...
		case LVAL_NUM: printf("%li", v->num); putchar('\n');			break;
		case LVAL_ERR: printf("Error: %s", v->err); putchar('\n');		break;
		case LVAL_SYM: printf("%s", v->sym); putchar('\n');				break;
		case LVAL_SEXPR: lval_expr_print(v, '(', ')'); putchar('\n');	break;
		case LVAL_QEXPR: lval_expr_print(v, '{', '}'); putchar('\n');	break;
...
lval* lval_eval(lenv* e, lval* v){
	...
	if(v->type == LVAL_SEXPR){
		...
		//若函数为清屏，则将返回值设为LVAL_CLS，在打印函数中识别并执行system("cls") 
		if(r->builtin == builtin_cls){
			r = lval_cls();
		}
		return r;
	}
